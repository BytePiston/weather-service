...
import com.verdant.weather.service.model.request_model.CoordinateRequestModel;
import com.verdant.weather.service.model.request_model.ZipCodeRequestModel;
import com.verdant.weather.service.model.response_model.WeatherResponse;
import com.verdant.weather.service.service.WeatherService;
...
public class CurrentWeatherController {
	...
	/*****************************************************************************************************************************
	 * API to retrieve the current weather of the city based on provided parameters;
	 * Either city ID or city name is required; State code and country code are optional;
	 ...
	 * <p>
	 *
	 * If id is provided, the API will return the current weather of the city; GET URL:
	 * api/v1/current/weather/city?id={id};
	 * <p>
	 * Else, if city name is provided, the API will return the current weather of the city
	 * based on below scenarios;
	 * <p>
	 * 1) If state code and country code are not provided, the API will return the current
	 * weather of the city; GET URL: api/v1/current/weather/city?name={cityName};
	 * <p>
	 * 2) If state code is provided, the API will return the current weather of the city
	 * in the state; GET URL:
	 * api/v1/current/weather/city?name={name}&stateCode={stateCode};
	 * <p>
	 * 3) If state code and country code are provided, the API will return the current
	...
	@Operation(summary = "Get current weather by city",
			description = "Get current weather by city ID or city name; State code and country code are optional")
	@ApiResponses({
			...
			@ApiResponse(responseCode = "400",
					...
					content = @Content),
			@ApiResponse(responseCode = "404", description = "Weather information not found for specified parameters",
					content = @Content) })
	public ResponseEntity<WeatherResponse> getCurrentWeatherByCity(
			...
			@Parameter(description = "State Code; Optional Field") @RequestParam Optional<String> stateCode,
			@Parameter(description = "Country Code; Optional Field") @RequestParam Optional<String> countryCode)
			throws ResourceNotFoundException {
		List<String> errorMessages = new ArrayList<>();

		if (id.isEmpty() && name.isEmpty()) {
			return ResponseEntity.badRequest()
				.body(WeatherResponse.builder().message("Either City ID or City name is required").build());
		}
			...
			try {
				...
				if (stateCode.isPresent() && countryCode.isPresent()) {
					...
					Optional<WeatherResponse> weatherResponse = weatherService
						.getCurrentWeatherByCityNameAndStateCodeAndCountryCode(name.get(), stateCode.get(),
								countryCode.get());
					return prepareWeatherResponse(weatherResponse);
				}
	 ...
	 * @param countryCode - Country code of the city; (Optional)
	 * @return ResponseEntity<WeatherResponse> - Current weather of the city based on
	 * below scenarios;
	 * <p>
	 * 1) If country code is not provided, the API will return the current weather based
	 * on zip code; GET URL: api/v1/current/weather/zip?zipCode={zipCode};
	 * <p>
	 * 2) If country code is provided, the API will return the current weather based on
	 * zip code and country code; GET URL:
	 * api/v1/current/weather/zip?zipCode={zipCode}&countryCode={countryCode};
	 */
	@GetMapping("/zip")
	@Operation(summary = "Get current weather by zip",
	...
	public ResponseEntity<WeatherResponse> getCurrentWeatherByZip(
		...
		if (countryCode.isPresent() && errorMessages.isEmpty()) {
			try {
				RequestValidationUtil.validateCountryCode(countryCode.get());
				weatherResponse = weatherService.getCurrentWeatherByZipCodeAndCountryCode(zipCode, countryCode.get());
				return prepareWeatherResponse(weatherResponse);
			}
			catch (ParameterValidationException e) {
				errorMessages.add(e.getMessage());
	 ...
	 * @return ResponseEntity<WeatherResponse> - Current weather of the location based on
	...
	public ResponseEntity<WeatherResponse> getCurrentWeatherByCoordinates(
			@Parameter(description = "Latitude; Required Field", required = true) @RequestParam Double latitude,
			@Parameter(description = "Longitude; Required Field", required = true) @RequestParam Double longitude)
			throws ResourceNotFoundException {
		List<String> errorMessages = new ArrayList<>();
		try {
		...
		if (errorMessages.isEmpty()) {
			Optional<WeatherResponse> weatherResponse = weatherService.getCurrentWeatherByCoordinates(latitude,
					longitude);
			return prepareWeatherResponse(weatherResponse);
		}
	 ...
	 * implementation; But can be used for future enhancements; The below methods are
	 * added to demonstrate the usage of POST as GET method to retrieve the current
	 * weather;
	 ****************************************************************************************************************************/

	/**
	 * POST as GET method to retrieve the current weather of the city based on provided
	 * parameters as JSON request;
	 * <p>
	 * POST URL: api/v1/current/weather/city;
	 * @param cityModel - CityRequestModel - Request model for city name, state code and
	 * country code;
	 * @return ResponseEntity<WeatherResponse> - Current weather of the city based on
	 * following scenarios;
	 * <p>
	 ...
	 * 2) If state code is provided, the API will return the current weather of the city
	 * in the state; GET URL:
	 * api/v1/current/weather/city?name={cityName}&stateCode={stateCode};
	 * <p>
	 * 3) If state code and country code are provided, the API will return the current
	 * weather of the city in the state of the country; GET URL:
	 * api/v1/current/weather/city?name={cityName}&stateCode={stateCode}&countryCode={countryCode};
	 */
	@PostMapping("/city")
	@Operation(summary = "POST as GET: Get current weather by city request model",
			description = "POST as GET: Get current weather by city name, state code and country code; State code and country code are optional")
	@ApiResponses({
			@ApiResponse(responseCode = "200",
					content = { @Content(mediaType = "application/json",
							schema = @Schema(implementation = WeatherResponse.class)) }),
			...
			@ApiResponse(responseCode = "404",
					description = "Weather information not available for specified parameters", content = @Content) })
	public ResponseEntity<WeatherResponse> getCurrentWeatherByCityRequestModel(
		...
		else if (cityModel.hasName() && cityModel.getCountryCode() != null && cityModel.getStateCode() != null) {
			weatherResponse = weatherService.getCurrentWeatherByCityNameAndStateCodeAndCountryCode(cityModel.getName(),
					cityModel.getStateCode(), cityModel.getCountryCode());
		}
		else if (cityModel.hasName() && cityModel.getCountryCode() != null) {
			weatherResponse = weatherService.getCurrentWeatherByCityNameAndCountryCode(cityModel.getName(),
	 ...
	 * <p>
	 * POST URL: api/v1/current/weather/zip;
	 ...
	 */
	@PostMapping("/zip")
	@Operation(summary = "POST as GET: Get current weather by zip request model",
			description = "POST as GET: Get current weather by zip code and country code; Country code is optional")
	@ApiResponses({
			@ApiResponse(responseCode = "200",
					content = { @Content(mediaType = "application/json",
							schema = @Schema(implementation = WeatherResponse.class)) }),
	...
	public ResponseEntity<WeatherResponse> getCurrentWeatherByZipRequestModel(
		...
		if (zipCodeModel.getZipCode() != null && zipCodeModel.getCountryCode() != null) {
			weatherResponse = weatherService.getCurrentWeatherByZipCodeAndCountryCode(zipCodeModel.getZipCode(),
					zipCodeModel.getCountryCode());
		}
		else {
	 ...
	 * provided parameters as JSON request;
	 * <p>
	 * POST URL: api/v1/current/weather/coordinates;
	 * @param coordinateModel - Request model for latitude and longitude;
	 * @return ResponseEntity<WeatherResponse> - Current weather of the location based on
	 * provided latitude and longitude;
	 */
	@PostMapping("/coordinates")
	@Operation(summary = "POST as GET: Get current weather by coordinates request model",
			description = "POST as GET: Get current weather by coordinates; Latitude and Longitude are required fields")
	@ApiResponses({
			@ApiResponse(responseCode = "200",
					content = { @Content(mediaType = "application/json",
							schema = @Schema(implementation = WeatherResponse.class)) }),
	...
	public ResponseEntity<WeatherResponse> getCurrentWeatherByCoordinateRequestModel(
		...
		Optional<WeatherResponse> weatherResponse = weatherService
			.getCurrentWeatherByCoordinates(coordinateModel.getLatitude(), coordinateModel.getLongitude());
		return prepareWeatherResponse(weatherResponse);
	}

	// Helper method to prepare the weather response;
	private ResponseEntity<WeatherResponse> prepareWeatherResponse(Optional<WeatherResponse> weatherResponse)
			throws ResourceNotFoundException {
		if (weatherResponse.isPresent()) {
			WeatherResponse response = weatherResponse.get();
			return ResponseEntity.ok(response);
		}
		throw new ResourceNotFoundException("Weather information not available for specified parameters.", null, false,
				false);
	}
