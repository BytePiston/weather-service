...
class CurrentWeatherControllerTest {
  ...
  static void setUp() throws IOException {
    naplesWeatherData.setCurrentWeather(
        objectMapper.readValue(
            new File("src/test/resources/weather_data/Naples.json"), WeatherModel.class));
    beverlyWeatherData.setCurrentWeather(
        objectMapper.readValue(
            new File("src/test/resources/weather_data/Beverly_Hills.json"), WeatherModel.class));
    quanticoWeatherData.setCurrentWeather(
        objectMapper.readValue(
            new File("src/test/resources/weather_data/Quantico.json"), WeatherModel.class));
  }
  ...
  void getCurrentWeatherByCityNamePositiveScenario() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/current/weather/city?name=" + "Naples"))
  ...
  void getCurrentWeatherByCityNameNegativeScenario() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/current/weather/city?name=" + "Lagos"))
  ...
  void getCurrentWeatherByCityIdPositiveScenario() throws Exception {
    ...
    mockMvc.perform(get("/api/v1/current/weather/city?id=" + "4164138")).andExpect(status().isOk());
  }
  ...
  void getCurrentWeatherByCityIdNegativeScenario() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/current/weather/city?id=" + 0))
  ...
  void getCurrentWeatherByZipCodePositiveScenario() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/current/weather/zip?zipCode=" + "22134"))
        .andExpect(status().isOk());
  }
  ...
  void getCurrentWeatherByCoordinatesPositiveScenario() throws Exception {
    ...
    mockMvc
        .perform(
            get(
                "/api/v1/current/weather/coordinates?latitude="
                    + 80.7128
                    + "&longitude="
                    + 80.0060))
        .andExpect(status().isOk());
  ...
  void getCurrentWeatherByCityRequestModelPositiveScenario() throws Exception {
    when(weatherService.getCurrentWeatherByCityNameAndCountryCode("Naples", "US"))
        .thenReturn(Optional.of(naplesWeatherData));
    mockMvc
        .perform(
  ...
  void getCurrentWeatherByCityRequestModelNegativeScenario() throws Exception {
    when(weatherService.getCurrentWeatherByCityNameAndCountryCode("Lagos", "NG"))
        .thenReturn(Optional.empty());
    mockMvc
        .perform(
            post("/api/v1/current/weather/city")
