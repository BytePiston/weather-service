...
import com.verdant.weather.service.entity.CitySearchTracker;
import com.verdant.weather.service.model.response_model.CityTrackerResponse;
import com.verdant.weather.service.repository.CityTrackerRepository;
...
class CityTrackerServiceImplTest {
	...
	@MockBean
	private CityTrackerRepository cityTrackerRepository;
	...
	void setUp() throws IOException {
		...
		ArrayNode jsonNode = (ArrayNode) objectMapper
			.readTree(new File("src/test/resources/weather_data/AllCityTrackerResponse.json"));
		for (JsonNode node : jsonNode) {
			CitySearchTracker citySearchTracker = CitySearchTracker.builder()
		...
		JsonNode node = objectMapper
			.readTree(new File("src/test/resources/weather_data/EdmontonCityTrackerResponse.json"));
		...
		Page<CitySearchTracker> page = convertArrayListToPage(allCityTrackerResponse, PageRequest.of(0, 3));
	...
	void getAllCityTrackerPositiveScenario() {
		...
		when(cityTrackerRepository.findById(any())).thenReturn(Optional.of(allCityTrackerResponse.get(0)));
		...
		page.forEach(citySearchTracker -> {
			cityTrackerResponses.add(CityTrackerResponse.builder()
	...
	void getAllCityTrackerPositiveScenarioWithOnePageSize() {
		Page<CitySearchTracker> page = convertArrayListToPage(allCityTrackerResponse, PageRequest.of(0, 1));
		when(cityTrackerRepository.findAll(any(Pageable.class))).thenReturn(page);
		List<CityTrackerResponse> cityTrackerResponses = new ArrayList<>();
		page.forEach(citySearchTracker -> {
			cityTrackerResponses.add(CityTrackerResponse.builder()
	...
	void getAllCityTrackerNegativeScenarioWithInvalidPageInput() {
		...
		assertThrows(IllegalArgumentException.class, () -> cityTrackerService.getAllCityTracker(PageRequest.of(0, 0)));
		assertThrows(IllegalArgumentException.class, () -> cityTrackerService.getAllCityTracker(PageRequest.of(0, -1)));
	}

	// Helper method to convert List to Page
	public static Page<CitySearchTracker> convertArrayListToPage(List<CitySearchTracker> list, Pageable pageable) {
